...
## Review follow-up: closed turn with an unsettled tool

Reproduced the reported defect using the actual native ToolCallTree in the local browser fixture: `turn.status=closed`, root still a RunningToolCall (no synthetic result). Before the fix, expanded parent was `interrupted` with no data-running, but the child still had the new shimmer. The CSS now admits that shimmer only under `.group[data-running="true"]`; the native pseudo-glare suppression remains unconditional inside details. Neither the source root nor event state is changed.

After the fix, the same expanded case has parent `interrupted`, native child `data-state=running` preserved, total descendant animation count **0**, child animation `none`, and child `::after` animation/content both `none`. Switching the fixture back to an open turn restores exactly two animations (header and tool), with the tool at `2s / steps(48)` and old pseudo-glare still absent. This is native-component browser evidence, not installed acceptance. Focused activity-fold suite: **12/12 passed**; plugin build passed. Follow-up logs are `work/2.0.18/message-flow/orphan-tests.log` and `orphan-build.log`; initial delivery logs are unchanged. Fixture's added “关闭轮次但工具未结算” control reproduces this case.
