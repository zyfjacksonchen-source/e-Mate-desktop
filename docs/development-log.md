...
- Focused Shell/gallery/task-details checks and the integrated source build passed. The Desktop native check subsequently exposed an external `fflate` import in the canvas host; bundling that private dependency fixed native Profile startup. The full Desktop check passed after that fix. Pet direction artwork and the 30 office scenes remain under acceptance. No 2.0.18 installed or public-release gate is closed by these changes.
