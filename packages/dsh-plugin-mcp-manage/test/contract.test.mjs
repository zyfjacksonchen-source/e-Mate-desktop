...
test('knowledge operation permits same Xin subject refresh but cannot follow a different authorized Xin account', async t => {
  ...
  h.xinAuthority({ tenant_id: 'xin', user_id: 1, principal_id: 300 })
  assert.equal((await h.owner.ensure({}, { reauthorize: true })).state, 'ready')
  await assert.rejects(operation.call('get_knowledge_compilation', {}))
  assert.equal(h.knowledgeCalls.length, 3)
})
...
test('an old model turn cannot first dispatch under a newly authorized Xin subject or after switching back', async t => {
  const h = await nativeXinHarness(t)
  const old = await h.agent('late-model-turn'); await h.owner.ensure(old.exec)
  const invoke = () => h.runtime.tools.execute({ callId: 'old-write', name: `mcp__${XIN_SERVICE}__mutate_project`, arguments: {}, agent: old.agent, signal: new AbortController().signal })
  h.xinAuthority({ tenant_id: 'another-xin-tenant', user_id: 200, principal_id: 201 })
  assert.equal((await h.owner.ensure({}, { reauthorize: true })).state, 'ready')
  assert.equal((await invoke()).isError, true)
  assert.equal((await h.owner.ensure(old.exec)).state, 'unavailable')
  h.xinAuthority({ tenant_id: 'xin', user_id: 1, principal_id: 301 })
  assert.equal((await h.owner.ensure({}, { reauthorize: true })).state, 'ready')
  assert.equal((await invoke()).isError, true)
  assert.throws(() => h.owner.captureKnowledge(old.exec))
  assert.deepEqual(h.writes, [])
  const next = await h.agent('fresh-model-turn')
  assert.equal((await h.owner.ensure(next.exec)).state, 'ready')
  const allowed = await h.runtime.tools.execute({ callId: 'old-write', name: `mcp__${XIN_SERVICE}__mutate_project`, arguments: {}, agent: next.agent, signal: new AbortController().signal })
