...
This vendored component is pinned to upstream `f3a8845dd4c863072b0ae555cdb6a58076c165e7` and e-Mate's existing Harness `0.1.5-rc.1`. Host imports resolve through the declared Base ABI; Client registers Definitions through the native `uiConversation` registry, reads the Chat target through the framework `useChat` hook, and projects native hidden Chat nodes whose root-call contexts fold PTC subcalls by `rootCallId`. The mature Gateway, Worker, Render Machine and Viewer remain their original owners.
