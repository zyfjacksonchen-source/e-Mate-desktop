...
For several `enterprise_knowledge` import or compilation batches in one native user message, supply a stable `batch_key` such as `public-001` for each batch. Reuse that key and the full original request when retrying; changed content under the same key still conflicts. An omitted key preserves existing single-batch operation IDs. Unknown results must be queried by the returned `operation_id`, never replayed with a new key. Batch keys do not change public-import consent or project permissions.
