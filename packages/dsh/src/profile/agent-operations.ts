...
const batchImageGuidance = 'Call `imagegen` in the current Agent once per image output, new or edited. Independent outputs may use concurrent calls to the same Tool. Dependent edits wait for the preceding real output. Give explicit source IDs for each edit or fusion and explain their roles. `image_batch` remains available when durable batch tracking is useful; it is not required for multiple outputs. Do not delegate merely to invoke image generation. Never infer source images, replay unknown requests, switch models, or fall back. If the native Tool is unavailable, report that capability failure. Native batch children call `imagegen` once with their exact admitted arguments. Successful images remain valid when sibling tasks fail; report each failure once and do not create replacements automatically. Never display attachment IDs, hashes, receipt pointers, child Session IDs, Job IDs, or sha256 values as image results.'
