...
describe('e-Mate 2.0.17 composer projection', () => {
  ...
  it('connects Xin directly and displays verified binding/permissions without editing the chat draft', async () => {
    ...
    const disconnectXin = vi.fn(async () => { current = disconnected; return disconnected })
    ...
    await waitFor(() => expect(screen.getByRole('button', { name: '断开并忘记芯助手' }).hasAttribute('disabled')).toBe(false))
