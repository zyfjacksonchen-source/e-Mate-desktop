...
import { SkillRow } from '../../../../../../upstream/deepseek-harness/packages/client/ui-skill/src/client/SkillRow.tsx'
...
it('leaves production process rendering with native Skill locale, injected faces and per-row failure isolation', async () => {
  const source = readFileSync(resolve('src/client/index.ts'), 'utf8')
  expect(source).not.toContain('registerActivityFold')
  expect(source).not.toContain('registerMessageModeSettings')
  const core = new SlotCore()
  const namespaces: string[] = [], injectedSessions: string[] = [], errors: string[] = []
  const info = { sessionId: 'native-skill-session', hooks: {}, props: {} }, locale = { revision: 0 }
  const host: any = {
    subscribe: (key: string, fn: () => void) => core.subscribe(key, fn), getVersion: (key: string) => core.getVersion(key),
    entriesOf: (key: string) => core.entries(key), entriesOfSlot: (key: string) => core.entriesOfSlot(key),
    reportEntryError: (key: string, entry: any, error: unknown, options: any) => { errors.push(key); core.reportEntryError(key, entry, error, options) },
    specOf: (key: string) => core.specDynamic(key), isLive: (entry: any) => core.isLive(entry), storeOf: () => undefined,
    locale: { getSnapshot: () => locale, subscribe: () => () => {}, bind: (namespace: string) => { namespaces.push(namespace); return (key: string) => key } },
    sessions: { list: { getSnapshot: () => ({}), subscribe: () => () => {} }, provideInfo: { getSnapshot: () => info, subscribe: () => () => {} } },
    workspaces: { list: { getSnapshot: () => ({}), subscribe: () => () => {} } },
  }
  const node = { kind: 'tool-call', data: { root: {
    callId: 'call-skill|fc-item', name: 'skill', argsRaw: '{"name":"vision-tools"}', subCalls: [
      { callId: 'bad', name: 'broken', argsRaw: '{}', subCalls: [] },
    ],
  } } }
  core.register({ name: 'root', children: { 'conversation.chat.node': { kind: 'keyed', scope: 'session' } } } as any,
    (({ renderSlot }: any) => <SessionProvider>{() => <>
      {renderSlot('conversation.chat.node', { node, inspectCall: () => {} }, { entryKey: 'tool-call' })}
      <button>已生成图片</button>
    </>}</SessionProvider>) as any)
  core.register({ name: 'conversation.chat.node', key: 'tool-call', locale: 'conversation', children: { 'tool.call.toolview': { kind: 'keyed', scope: 'session' } } } as any, ToolCallTree as any)
  core.register({ name: 'tool.call.toolview', key: 'skill', locale: 'skill', inject: (sessionId: string) => ({ injectedSession: sessionId }) } as any,
    ((props: any) => { injectedSessions.push(props.injectedSession); return <SkillRow {...props} /> }) as any)
  core.register({ name: 'tool.call.toolview', key: 'broken' } as any, (() => { throw new Error('isolated tool row') }) as any)
  const error = vi.spyOn(console, 'error').mockImplementation(() => {})
  const view = render(<>{createSlotRenderer().renderRoot(host, {})}</>)
  await act(async () => {})
  expect(screen.getByText('vision-tools')).toBeTruthy()
  expect(namespaces).toContain('skill')
  expect(injectedSessions).toContain('native-skill-session')
  expect(errors).toEqual(['tool.call.toolview'])
  expect(view.container.querySelector('[data-slot-error="tool.call.toolview"]')).not.toBeNull()
  expect(core.entriesOfSlot('conversation.chat.node')).toHaveLength(1)
  expect(screen.getAllByRole('button', { name: '已生成图片' })).toHaveLength(1)
  error.mockRestore()
})
