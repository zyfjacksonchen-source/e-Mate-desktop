...
describe('Codex-like process fold', () => {
  it('keeps natural assistant messages native while folding Think, Tool and GenUI detail', () => {
    ...
    const header = screen.getByRole('button', { name: /正在为你整理页面。/u })
    expect(header.getAttribute('aria-expanded')).toBe('false')
    expect(header.querySelector('[data-emate-brain-icon]')).toBeNull()
    expect(screen.getAllByText('正在为你整理页面。').length).toBeGreaterThan(0)
    ...
    expect(view.container.querySelector('[data-emate-activity-fold]')).not.toBeNull()
  ...
  it('projects one process group without rewriting DSH nodes', () => {
    ...
    expect(activityFoldSummary([...nodes.keys()], nodes, nodes.get('context'))).toMatchObject({
      turn: 2,
      headerKey: 'message',
      toolCount: 1,
      reasoningCount: 0,
    ...
    expect(cssSource).toContain('.entry + .entry')
...
it('keeps manual running collapse through streaming, new steps, completion, turns and same-runtime remount', () => {
  ...
  let header = screen.getByRole('button', { name: /正在处理/ })
  ...
  header = screen.getByRole('button', { name: /正在处理/ })
  ...
  header = screen.getByRole('button', { name: /已完成/ })
  expect(header.getAttribute('aria-expanded')).toBe('false')
  ...
  expect(screen.getByRole('button', { name: /正在处理/ }).getAttribute('aria-expanded')).toBe('false')
...
it.each(['fallback', 'dry', 'assembly'] as const)('keeps the process fold elected after a faulty atomic ToolView: %s', async (mode) => {
  ...
  let view = mount()
  expect(screen.getAllByText('PPTX 已成功导出，五页预览也已逐页检查。现在做最后文件核验。')[0]).toBeTruthy()
  expect(screen.getByText('文件已完成，请查看最终结果。')).toBeTruthy()
  if (mode === 'assembly') {
    await expect(act(async () => fireEvent.click(screen.getByRole('button', { name: /PPTX 已成功导出/ })))).rejects.toBe(crash)
    expect(errors).toEqual([])
    return
  }
  await act(async () => fireEvent.click(screen.getByRole('button', { name: /PPTX 已成功导出/ })))
...
it('stops shimmer on native closed boundaries even when an old tool result never arrived', () => {
  const node = { key: 't', kind: 'tool-call', location: { kind: 'step', turn: { turn: 1, status: 'closed' } }, data: { root: { callId: 'pending' } } }
  const summary = activityFoldSummary(['t'], new Map([['t', node]]), node)
  expect(summary).toMatchObject({ running: false, state: 'interrupted' })
  const interrupted = { ...node, data: { root: { callId: 'pending', interrupted: true } } }
  expect(activityFoldSummary(['t'], new Map([['t', interrupted]]), interrupted)).toMatchObject({ running: false, state: 'interrupted' })
})

it('moves streaming prose into the process on tool admission and keeps the trailing answer native', () => {
  const loc = { kind: 'step', turn: { turn: 3, status: 'open' } }
  const progress = { key: 'p', kind: 'assistant-step', location: loc, data: { status: 'settled', blocks: [{ kind: 'text', text: '检查文件' }] } }
  expect(activityFoldSummary(['p'], new Map([['p', progress]]), progress)).toBeNull()
  const tool = { key: 't', kind: 'tool-call', location: loc, data: { root: { callId: 'read' } } }
  const nodes = new Map<string, any>([['p', progress], ['t', tool]])
  expect(activityFoldSummary(['p', 't'], nodes, progress)).toMatchObject({ progress: '检查文件', processKeys: ['p', 't'], running: true })
  nodes.set('a', { ...progress, key: 'a', data: { status: 'running', blocks: [{ kind: 'text', text: '检查结果' }] } })
  expect(activityFoldSummary(['p', 't', 'a'], nodes, progress)).toMatchObject({ processKeys: ['p', 't'], finalKey: 'a' })
})
