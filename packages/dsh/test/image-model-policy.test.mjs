...
test('live model policy accepts only Flare with no legacy or fallback route', () => {
  assert.equal(validateModelPolicy(policy, 'tenant:user', now).image_primary_model_id, 'gpt-image-2.5-flare')
...
test('native durable policy reads authenticated old formats and writes Flare without fallback', async () => {
  ...
  try {
    for (const legacy of [old, { ...old, image_fallback_upstream_model_id: 'gpt-image-2', allowed_model_ids: [...old.allowed_model_ids, 'gpt-image-2'] }]) {
      ...
      assert.equal(written.image_primary_model_id, 'gpt-image-2.5-flare')
      assert.deepEqual(written.allowed_model_ids, ['gpt-5.6-luna', 'gpt-image-2.5-flare'])
      ...
      ctx.storageDomain.open = async () => ({ close() {}, table() { return { get() { return { ...sign(legacy), image_primary_model_id: 'gpt-image-2.5-flare' } }, async put() { assert.fail('tampered policy must never be written') } } } })
