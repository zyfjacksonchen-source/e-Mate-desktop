...
test('queue, current job failures and delivered output use native status only',()=>{
  ...
  list.set({...list.getSnapshot(),jobsBySession:{a:[{status:'failed',startedAt:90}]}});assert.equal(deriveScene(projection.getSnapshot()),'queue')
  list.set({...list.getSnapshot(),jobsBySession:{a:[{status:'failed',startedAt:101}]}});assert.equal(deriveScene(projection.getSnapshot()),'error')
