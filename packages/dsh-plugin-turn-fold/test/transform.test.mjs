...
test('applies all three vendored patches in memory against the pinned build', () => {
  ...
  assert.equal(result.pinnedSha256, TARGET_BUNDLE_SHA256)
...
test('the checker and the driver refuse the same bundle through one shared pin', () => {
  ...
  assert.equal(checkerRefusal.report.pinnedSha256, TARGET_BUNDLE_SHA256)
  ...
  assert.equal(driverRefusal.report.pinnedSha256, TARGET_BUNDLE_SHA256)
