 ...
 * bundle before it is rewritten, so it repeats the check here, against the single
 * `TARGET_BUNDLE_SHA256` both callers import from `./select.cjs`. Hashing happens before
 ...
 * @returns {{ actualSha256: string, pinnedSha256: string }} the verified identity.
...
function assertVerifiedBundle(sourceBytes, target) {
  ...
  if (evaluation.result.ok !== true) {
    const error = new Error(
      ...
      'the new build, then move TARGET_BUNDLE_SHA256 in packages/dsh-plugin-turn-fold/src/select.cjs',
    )
    error.report = {
      applied: [],
      ...
      pinnedSha256: TARGET_BUNDLE_SHA256,
      target,
    }
    throw error
  }
  return { actualSha256, pinnedSha256: TARGET_BUNDLE_SHA256 }
}
 ...
 * @returns {{ text: string, sourceText: string, target: object, patches: Array<object>, sourceSha256: string, pinnedSha256: string, bundleVerified: boolean, sha256: string }}
 *   the transformed bundle text; nothing is written to disk.
 */
function transformClientBundle(options = {}) {
  const harnessRoot = findHarnessRoot(options)
  const target = locateTarget(harnessRoot, options.target ?? TARGET)
