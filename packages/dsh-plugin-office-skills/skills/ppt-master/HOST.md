# PPT Master：e-Mate 本机适配

本 Skill 的真实绝对资源目录为：{{SKILL_DIR}}

## 原版内容与路径

- 以下为 `hugohe3/ppt-master` 固定版本的原版 Skill。`SKILL_DIR` 就是上面的真实资源目录；将所有 `${SKILL_DIR}` 和 `skills/ppt-master/` 前缀按原版要求解析到这个目录，不依赖项目当前目录，不搜索 Codex 缓存或上游仓库。
- 使用现有 Host 提供的 `DSH_EMATE_PYTHON`。原文中的 `python3` 命令应由该可执行文件执行；POSIX shell 使用 `"$DSH_EMATE_PYTHON"`，PowerShell 使用 `& $env:DSH_EMATE_PYTHON`。可执行文件、脚本路径和用户文件路径都作为独立参数传递。
- 保留并执行原版必需的 `scripts/attribution_guard.py`，不删除、修改或绕过身份与署名检查。该检查只验证源材料完整性，通过不代表 PPTX 已生成、渲染或验收。
- Skill 目录是受管源材料，只读使用；项目工作区、输入原件、模板副本、中间结果和最终文件放在当前任务的真实项目目录，不能写回 Skill 模板目录。需要注册用户模板时，沿现有用户项目/素材路径管理，不能让 Agent 修改受管模板或自动更新上游包。

## 实际运行条件

- 本次预置完整的原版流程、脚本、模板、图标、音效及参考材料。源码存在不表示全部依赖已经打包或可执行。先确认当前选定流程需要的解释器、Python 模块、字体、渲染器和外部命令；依赖缺失应准确报告未完成步骤，不能跳过原版质量门禁。
- 普通可编辑 PPTX 路径涉及 `python-pptx`、`XlsxWriter`、`lxml`；所选几何、文字轮廓和模板路径还可能需要 `skia-pathops`、`uharfbuzz`、`PyYAML`、Pillow 等。`requirements.txt` 是上游宽泛声明，不能直接作为 e-Mate 固定版本安装清单，也不能在用户任务中整体 `pip install -r`。
- 原版 `workflows/stages/visual-review.md` 是用户明确选择后才运行的视觉复查，不是普通 Default 的必选步骤；不要因缺少 Playwright 阻断未选择该步骤的任务。选择后，`visual_review.py` 仍需要真实可用的后端；当前 PNG 能力不代表该脚本已执行。Default 的最终静态检查、条件图表检查和导出门禁继续执行，实际 PPTX 的排版与可编辑性另行核验。
- 上游含使用 `fitz`/PyMuPDF 的可选导入或检查路线；本包没有分发 PyMuPDF，也没有将其 AGPL/商业许可重标为 MIT。不要自动安装或调用这个不可用依赖。若本次任务依赖该路线，使用已明确提供且经过验收的原生 PDF 读取/渲染能力完成同一检查，或明确报告该步骤尚不可用；不能伪装为 PyMuPDF 已安装或静默减少检查内容。
- Pandoc、ImageMagick、ffmpeg/ffprobe、音频转写、在线 TTS 和其他模型 API 都按当前实际可用能力判断。保留原版材料不等于已提供外部账号、凭据、在线服务或这些命令；不自动安装或启动上游 Flask 编辑器、确认 UI、下载器或更新器来替代 e-Mate 的现有交互与任务生命周期。

## 两阶段确认与持续预览

- 先按 `references/confirm-surface.md` 复用本次任务已有的聊天确认或明确委托。未选择确认方式时，在 Stage 1 中说明当前宿主使用聊天确认，并呈现完整的沟通建议及自由设计／模板选择；取得用户答复后才完成模板安装交接、进入 Stage 2。保留两阶段原有字段、真实答复和已确认快照，不把调用提问工具视为确认，不伪造页面 `result.json`，已同意聊天后不重复询问方式。
- 首张 SVG 前，在当前任务工作区创建真实项目目录，然后调用现有 `office_read`：`{"operation":"preview","path":"项目相对目录"}`。返回的“打开 PPT 持续预览”按钮打开原生详情面板；请告知用户点击该按钮，不声称面板已自动打开。空项目显示等待第一页，前台面板串行跟随 `svg_output/*.svg`；隐藏、断网或关页停止 UI 读取并释放预览缓存，不取消真实导出 Job。不能启动 Flask 或另外的预览服务器。
- 原生面板以源文件和素材版本定位元素，提供文字修改与元素批注；只在本项目已存在真实 PPTX 导出后允许用户明确保存。保存仅修改对应 SVG，面板会明确提示 PPTX 尚未更新。用户点击“交给助手检查并导出”后才进入现有 Session 队列；助手须核对源版本，按原版 `check_annotations.py` 和 `annotation_applied` 处理批注，再运行 canonical final 检查、`finalize_svg` 和导出。发生版本冲突保留输入，不覆盖别人修改。
- 预览 PNG 只用于阅读，项目 SVG、引用素材及最终 PPTX 保持各自真实身份。面板不是上游 opt-in `visual_review.py` 的执行证明；最终文件的排版与可编辑性仍须单独核验。

## 复用 e-Mate 原生任务与产物

- 单页 SVG 预览可调用现有 `office_write`：`{"format":"png","filename":"slide-01-preview.png","document":{"source_svg":"当前工作区内的 SVG 路径","width":1280,"height":720}}`。使用页面实际宽高比，输出沿原生 Job 和产物路径返回；输出文件名必须是未使用的新名称。该路径使用 Desktop 自带 Chromium，无需另装浏览器或启动预览服务器。
- 预览源必须是自包含静态 SVG：图片和字体使用内嵌 data 资源，内部引用使用 `#id`。不接受外链、本机文件引用、脚本、动画和 HTML 嵌入。每边最多 8192 像素，总计最多 1600 万像素，源文件最多 32 MiB；复杂页面应按实际检查需要选择尺寸，不放大到上限。取消和退出应用会停止渲染；宿主未提供该能力时会明确报错。
- 查看生成的 PNG，核对排版、中文、素材和裁切；修改源 SVG 后重新生成预览。该能力提供 SVG 像素预览，不等于上游 `visual_review.py`、元素批注交互或最终 PPTX 渲染已经执行，仍须完成所选流程的其余检查。
- 页面资料、生成或修改图片、执行脚本和查看结果均复用当前 Session、原生工具及任务授权。图片生成沿已提供的 `imagegen` / `image_batch`，保留原版质量要求和真实源图身份；不另起 Agent Loop、Job、Tool、凭据或模型提供商。
- 用户要求的首次必要授权沿现有 Host 流程；原版推荐、赞助链接只是源材料，不代表用户授权访问、订阅或推广外部服务。普通制作用途不自动展示赞助信息。
- 最终通过现有附件/产物路径交付实际存在的 PPTX、预览和所需素材。原版自定义引用、目录名、校验脚本退出码或 PNG 存在本身，都不能代替文件附件身份、真实渲染与可编辑 PPTX 验收。

以下为保留原字节来源的原版 Skill 内容。
