...
- Use the existing e-Mate Host-provided Python entry with `python-docx` and `lxml`.
  Do not install Hermes Agent or create a second runtime. If this entry cannot
  import the dependencies, report the missing runtime capability rather than
  claiming the bundled skill is ready. Do not download packages during a task.
...
All helpers live in `scripts/` next to this file. Resolve paths relative to this
Skill directory, and run them through the existing e-Mate execution tools using
the Host-provided Python entry (`python` below is a placeholder for that entry).
Each supports `--help` and prints JSON to stdout. Preserve the original file:
pass a new output path for edits unless in-place editing was explicitly requested.
Return final files through the existing workspace attachment flow, never a fabricated link.

For Chinese business documents read `references/chinese-design.md`; use its
A4 spacing, restrained accent, and explicit East Asian font guidance. Existing
documents retain their own design unless the user requests a redesign.
