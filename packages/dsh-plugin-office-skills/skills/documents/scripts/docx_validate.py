...
def validate(path: str) -> dict:
    ...
    for rels_name in [n for n in names if n.endswith(".rels")]:
        ...
        for rel in root.iter(f"{{{PR}}}Relationship"):
            ...
            if mode == "External":
                continue
            resolved = _rel_target(source_part, target)
    ...
    for part_name in sorted(n for n in names if n.startswith("word/") and n.endswith(".xml")):
        try:
            part_root = doc_root if part_name == "word/document.xml" else etree.fromstring(zf.read(part_name))
        except etree.XMLSyntaxError as exc:
            _issue(issues, "error", "bad-part-xml", f"{part_name}: {exc}")
            continue
        part_rels = rel_ids_by_source.get(part_name, {})
        for el in part_root.iter():
            for attr in (f"{{{R}}}id", f"{{{R}}}embed", f"{{{R}}}link"):
                rid = el.get(attr)
                if rid and rid not in part_rels:
                    _issue(issues, "error", "unresolved-reference",
                           f"{part_name} references {rid} with no relationship")
