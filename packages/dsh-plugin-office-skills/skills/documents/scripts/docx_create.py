...
Spec (JSON object):
{
  ...
  "styles": [{"name": "MyStyle", "base": "Normal", "font": "Arial",
              ...
              "east_asia_font": "Noto Sans CJK SC", "space_after_pt": 6,
              "line_spacing": 1.35, "keep_with_next": false}],
  "blocks": [
...
def add_styles(doc, styles: list) -> None:
    for s in styles:
        style = (doc.styles[s["name"]] if s["name"] in doc.styles
                 else doc.styles.add_style(s["name"], WD_STYLE_TYPE.PARAGRAPH))
        if style.type != WD_STYLE_TYPE.PARAGRAPH:
            raise ValueError("only paragraph styles can be updated")
        if s.get("base"):
            style.base_style = doc.styles[s["base"]]
        font = style.font
        if s.get("font"):
            font.name = s["font"]
        if s.get("east_asia_font"):
            style.element.get_or_add_rPr().get_or_add_rFonts().set(
                qn("w:eastAsia"), s["east_asia_font"])
        paragraph = style.paragraph_format
        for key in ("space_before_pt", "space_after_pt"):
            if key in s:
                setattr(paragraph, key.removesuffix("_pt"), Pt(s[key]))
        if "line_spacing" in s:
            paragraph.line_spacing = float(s["line_spacing"])
        if "keep_with_next" in s:
            paragraph.keep_with_next = bool(s["keep_with_next"])
        if s.get("size_pt"):
