...
The 22 tests use pinned 0.1.5-rc.1 ToolRuntime, LocalJobRegistry, LocalAttachmentStore and Session with fixture provider responses. A loopback test also runs the actual e-Mate model-gateway router, schema, authorization and InMemoryUsageStore, checking separate request finalization and multi-source edits. These checks establish source/native and local gateway behavior only, not installed behavior, live provider success or remote billing settlement.
