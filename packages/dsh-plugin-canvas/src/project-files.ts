...
export async function canvasDirectory(root: string, create = false): Promise<string> {
  let current = root
  for (const part of ['.e-mate', 'canvas']) {
...
async function assertDirectory(root: string, directory: string): Promise<void> {
  if (await canvasDirectory(root) !== directory) reject('画布目录已变化。', 'conflict')
}
...
async function targetFile(fs: NativeFiles, root: string, directory: string, name: string) {
...
async function atomic(fs: NativeFiles, root: string, directory: string, name: string, bytes: Uint8Array) {
  const target = await targetFile(fs, root, directory, name)
...
export async function loadProject(root: string, id: string): Promise<ProjectReceipt | null> {
  identifier(id)
  let directory
  try { directory = await canvasDirectory(root) } catch (error) { if (missing(error)) return null; throw error }
...
export async function saveProject(fs: NativeFiles, root: string, value: unknown, expectedRevision: string | null): Promise<ProjectReceipt> {
  const project = validateProject(value)
  if (expectedRevision !== null && expectedRevision !== '' && !/^[0-9a-f]{64}$/u.test(expectedRevision)) reject('保存版本无效。')
  const directory = await canvasDirectory(root, true)
  const target = await targetFile(fs, root, directory, `${project.id}.json`)
  ...
  if (current) {
    ...
    if (valid) await atomic(fs, root, directory, `${project.id}.backup.json`, current)
    // Preserve corrupt bytes for diagnosis, retaining the valid backup.
    else await atomic(fs, root, directory, `${project.id}.corrupt-${sha256(current)}.json`, current)
  }
...
export async function listProjects(root: string): Promise<readonly { id: string; title: string; recovered: boolean; error?: string }[]> {
  let directory
  try { directory = await canvasDirectory(root) } catch (error) { if (missing(error)) return []; throw error }
  ...
  return await Promise.all(ids.map(async id => {
    try { const result = await loadProject(root, id); return { id, title: result?.project.title ?? id, recovered: result?.recovered ?? false } }
