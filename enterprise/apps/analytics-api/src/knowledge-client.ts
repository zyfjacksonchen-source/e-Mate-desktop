...
export function knowledgeRoute(method: string | undefined, url: URL, payload: unknown): { action: string; params: Record<string, unknown> } {
  ...
  const queries: Record<string, readonly string[]> = {
    '/catalog': [], '/sources': ['kind', 'offset', 'limit', 'corpus_revision'],
    '/benchmark': ['keyword'], '/graph': ['root_id', 'depth', 'limit', 'corpus_revision'],
  };
  if (method === 'GET') {
  ...
  if (method === 'POST' && (path === '/search' || path === '/benchmark')) {
    ...
    const keys = path === '/search' ? ['question', 'limit', 'layer', 'corpus_revision'] : ['media', 'industry', 'metric', 'period', 'source_id', 'marketing_purpose'];
...
export function createKnowledgeClient(rawConfiguration: KnowledgeConfiguration): KnowledgeReader {
  ...
  return { async read(principal, action, params, signal) {
    ...
    const body = Buffer.from(JSON.stringify({ actor: { enterprise_tenant_id: principal.tenantId, subject_id: principal.userId }, knowledge_tenant_id: configuration.tenantMappings[principal.tenantId], action, params }));
    if (body.length > 65536) throw new KnowledgeError(413, 'INVALID_QUERY');
    active++;
    try {
      return await new Promise<KnowledgeReply>((resolve, reject) => {
        const request = httpRequest({ socketPath: configuration.socketPath, path: '/read', method: 'POST', signal,
          ...
          response.on('end', () => {
            ...
            try {
              ...
              if (contentType !== 'application/json' || json.schema_version !== 1 || json.scope?.kind !== 'public' || !sha256.test(json.corpus_revision)) throw Error('invalid');
