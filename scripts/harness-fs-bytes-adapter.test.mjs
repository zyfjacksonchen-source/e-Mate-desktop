...
test('the integrated native ZIP producer refuses an imported hardlink through the guarded native FS owner', async t => {
  ...
  const api = adaptHarnessSessionExportSource(await fs.readFile(path.join(nativeRoot, 'upstream/deepseek-harness/packages/session-query/session-log-export/lib/index.js'), 'utf8'))
