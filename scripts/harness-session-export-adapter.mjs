...
export function adaptHarnessSessionExportSource(source) {
  ...
  change('\trememberAttachments(rootContent);\n\tyield {\n\t\tpath: SESSION_LOG_FILENAME,\n\t\tcontent: rootContent\n\t};', '\tconst emateRootContent = emateExportContent(rootContent);\n\trememberAttachments(emateRootContent);\n\tyield {\n\t\tpath: SESSION_LOG_FILENAME,\n\t\tcontent: emateRootContent\n\t};\n\tyield* emateExportFiles(deps, emateRootContent, sessionId, "", signal);', 'root')
  change('\t\t\t\trememberAttachments(content);\n\t\t\t\tyield {\n\t\t\t\t\tpath: \`subagents/${safeSessionIdSegment(id)}/${SESSION_LOG_FILENAME}\`,\n\t\t\t\t\tcontent\n\t\t\t\t};', '\t\t\t\tconst emateChildContent = emateExportContent(content);\n\t\t\t\trememberAttachments(emateChildContent);\n\t\t\t\tyield {\n\t\t\t\t\tpath: \`subagents/${safeSessionIdSegment(id)}/${SESSION_LOG_FILENAME}\`,\n\t\t\t\t\tcontent: emateChildContent\n\t\t\t\t};\n\t\t\t\tyield* emateExportFiles(deps, emateChildContent, id, \`subagents/${safeSessionIdSegment(id)}/\`, signal);', 'descendants')
  return source
}
