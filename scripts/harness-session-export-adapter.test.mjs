...
async function archive(deps, content, descendants = false, selected = zipOwner, signal = new AbortController().signal) {
  const raw = { filename: 'session.jsonl', content }
  ...
  assert.equal(raw.content, content, 'export must never mutate the stored artifact')
  return files
}
...
test('pinned native owner reproduces staged-image disclosure and ordinary-file omission; adapted ZIP closes both', async t => {
  ...
  assert.equal(new TextDecoder().decode(result['session.jsonl']), content.replace(line(staged('never-sent-secret')), ''))
})
...
test('descendant export uses its own workspace and filters drafts before native media deduplication', async t => {
  ...
  deps.sessionPersistence.readRaw = async () => ({ filename: 'session.jsonl', content: line(staged('removed')) + line(user()) + line(sentImage('shared')) })
  ...
  assert.equal(new TextDecoder().decode(result['subagents/child/session.jsonl']).includes('removed'), false)
...
test('real attachment-store digest IDs produce safe, reversible ZIP media basenames', async t => {
  ...
  const data = new Uint8Array(Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=', 'base64'))
  ...
  const ref = await saveImageFile(root, { data, mediaType: 'image/png', name: 'pixel.png' }, limits)
  ...
  assert.equal(new TextDecoder().decode(result['session.jsonl']), content)
