...
function summary(element, includeIndex) {
    ...
    if (element.title !== undefined)
        parts.push(JSON.stringify(element.title));
    else if (element.label !== undefined)
        parts.push(JSON.stringify(element.label));
...
export function diffElements(previous, current, maxBytes) {
    ...
    for (const [key, element] of before) {
        if (!after.has(key))
            lines.push(`- ${summary(element, false)}`);
    }
    for (const [key, element] of after) {
        const old = before.get(key);
        if (old === undefined)
            lines.push(`+ ${summary(element, true)}`);
        else if (state(old) !== state(element))
            lines.push(`~ ${summary(element, true)}`);
    }
    if (lines.length === 0)
